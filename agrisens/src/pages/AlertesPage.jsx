...
const STATUT_CLASS = {
  ouverte:   'badge-critical',
  acquittee: 'badge-warning',
  resolue:   'badge-resolue',
  ignoree:   'badge-offline',
};
    ...
    <div className="p-6 space-y-5">
        ...
        {['ouverte','acquittee','resolue','toutes'].map(s => (
          <button key={s} onClick={() => setFilter(s)}
            className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors capitalize ${
              filter === s ? 'bg-agri-600 text-white' : 'bg-gray-800 text-gray-400 hover:text-white'
            }`}>{s}</button>
        ))}
      </div>
      ...
      {loading ? (
        ...
        <div className="card text-center py-16">
          <CheckCircle size={40} className="text-agri-500 mx-auto mb-3" />
          ...
          {alertes.map(alerte => (
            <div key={alerte.id} className="card flex items-start gap-4">
              <AlertTriangle size={20} className={
                alerte.severite === 'critical' || alerte.severite === 'urgence'
                  ? 'text-red-400 mt-0.5 shrink-0'
                  : 'text-yellow-400 mt-0.5 shrink-0'
              } />
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  <span className={SEVERITE_CLASS[alerte.severite]}>{alerte.severite}</span>
                  <span className={STATUT_CLASS[alerte.statut]}>{alerte.statut}</span>
                  <span className="text-xs text-gray-500">
                    {alerte.device_uid} · {alerte.type_mesure}
                  </span>
                </div>
                <p className="text-white text-sm">{alerte.message}</p>
                <p className="text-xs text-gray-500 mt-1">
                  {format(new Date(alerte.created_at), "d MMM yyyy 'à' HH:mm", { locale: fr })}
                  {alerte.acquitte_par_nom?.trim() && ` · Acquitté par ${alerte.acquitte_par_nom}`}
                </p>
              </div>
